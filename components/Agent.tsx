...
const Agent = ({
  ...
  // Timer/ref used to detect end-of-questions from assistant transcripts
  const assistantEndTimerRef = useRef<number | null>(null);
  const ASSISTANT_END_DEBOUNCE_MS = 2500; // wait this long after last assistant final transcript
  ...
  useEffect(() => {
    ...
    const onMessage = (message: any) => {
      ...
      if (message.type === "transcript") {
        if (message.transcriptType === "interim") {
          ...
          // If assistant produced a final transcript, debounce to detect end-of-questions
          if (message.role === "assistant") {
            try {
              if (assistantEndTimerRef.current) {
                clearTimeout(assistantEndTimerRef.current);
              }
            } catch {}
            ...
            }, ASSISTANT_END_DEBOUNCE_MS) as unknown as number;
          }
        }
      }
    };
    ...
    return () => {
      ...
      try { if (assistantEndTimerRef.current) clearTimeout(assistantEndTimerRef.current); } catch {}
  ...
  useEffect(() => {
    ...
    const handleGenerateFeedback = async (messages: SavedMessage[]) => {
      // Prevent feedback submission if interviewId is missing
      if (!interviewId) {
        // Do not attempt to save feedback or redirect, just return silently
        return;
      }
      ...
      if (success && id) {
        ...
        setErrorMessage("Error saving feedback");
    ...
    const handleGenerateAndFeedback = async (messages: SavedMessage[]) => {
      try {
        // Attempt to create interview via API with Authorization header
        const res = await fetch("/api/vapi/generate", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": "Bearer vanakam-daaa",
          },
          body: JSON.stringify({
            type,
            role,
        ...
        if (result.success && result.interviewId) {
          ...
          setErrorMessage("Error creating interview");
    ...
    if (callStatus === CallStatus.FINISHED) {
      if (type === "generate") {
        // If in 'generate' mode, do NOT save feedback, just redirect
        handleGenerateAndFeedback(messages);
      } else if (interviewId) {
        // Only save feedback if NOT in 'generate' mode and interviewId exists
        handleGenerateFeedback(messages);
      }
      // If interviewId is missing, do nothing (prevents error)
    }
  ...
  const handleCall = async () => {
    ...
    try {
      ...
      // Use ASSISTANT_ID for creating interview, INTERVIEW_ASSISTANT_ID for speaking questions
      if (!questions || questions.length === 0) {
        // Start Assistant for interview creation/config collection
        ...
        // Start Interview Assistant for speaking questions
  ...
  return (
    <>
      {/* Gemini Question Generation Button */}

      {/* Display Gemini Questions if present */}
      {geminiQuestions.length > 0 && (
        <div className="w-full flex flex-col items-center mt-4">
          <div className="bg-muted rounded-xl px-8 py-4 shadow text-center text-lg text-foreground font-medium max-w-2xl">
            <div className="mb-2 font-bold">Gemini Questions:</div>
